...
namespace Bambo
{
	namespace BMath
	{
		...
		inline bool IsNearlyEqual(float v1, float v2, float tolerance = DEFAULT_TOLERANCE)
		{
			return std::abs(v1 - v2) < tolerance;
		}

		template<typename T>
		inline bool IsNearlyEqual(const Vector3<T>& vec1, const Vector3<T>& vec2, float tolerance = DEFAULT_TOLERANCE)
		{
			return vec1.X == vec2.X && vec1.Y == vec2.Y && vec1.Z == vec2.Z;
		}

		inline bool IsNearlyEqual(const Vector3f& vec1, const Vector3f& vec2, float tolerance = DEFAULT_TOLERANCE)
		{
			return IsNearlyEqual(vec1.X, vec2.X, tolerance) && IsNearlyEqual(vec1.Y, vec2.Y, tolerance) && IsNearlyEqual(vec1.Z, vec2.Z, tolerance);
		}

		template<typename T>
		inline bool IsNearlyEqual(const Vector2<T>& vec1, const Vector2<T>& vec2, float tolerance = DEFAULT_TOLERANCE)
		{
			return vec1.X == vec2.X && vec1.Y == vec2.Y;
		}

		inline bool IsNearlyEqual(const Vector2f& vec1, const Vector2f& vec2, float tolerance = DEFAULT_TOLERANCE)
		{
			return IsNearlyEqual(vec1.X, vec2.X, tolerance) && IsNearlyEqual(vec1.Y, vec2.Y, tolerance);
		}

		template<typename T>
		inline Vector2<T> ToVec2(const Vector3<T>& vec)
		{
			return Vector2<T>{ vec.X, vec.Y };
		}

		template<typename T>
		inline float Clamp(T from, T to, T value)
		{
		...
		inline float InverseLerpClamped(float a, float b, float t)
		{
		...
		inline void Sum(int& counter, float& total, float num)
		{
			++counter;
			total += num;
		}
		...
		inline void Sum(int& counter, float& total, float num, Args... args)
		{
			++counter;
			total += num;
		...
		template<typename... Args>
		inline float Average(Args... args)
		{
			float total = 0.0f;
			int32 counter = 0;
			Sum(counter, total, args...);
